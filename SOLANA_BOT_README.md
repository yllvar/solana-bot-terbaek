...
## 📁 Struktur Projek
```
solana-sniping-bot-main/
├── solana_bot/             # Kod sumber bot (SELAMAT)
│   ├── __init__.py
│   ├── config.py         # Pengurusan konfigurasi
│   ├── wallet.py         # Pengurusan dompet selamat
│   ├── monitor.py        # Pemantauan pool Raydium
│   ├── security.py       # Analisis keselamatan token
│   ├── transaction.py    # Pengurusan transaksi
│   ├── price_tracker.py  # Pemantauan harga
│   ├── triggers.py       # Take Profit / Stop Loss
│   └── raydium/          # Modul Raydium Swap
├── solana_bot_cli.py       # CLI Utama (JALANKAN INI)
├── bot_config.json       # Fail konfigurasi
├── requirements_safe.txt # Kebergantungan Python
└── SOLANA_BOT_README.md    # Dokumentasi ini
```
